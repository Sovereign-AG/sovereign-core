...
export async function POST(request: Request) {
  try {
    ...
    const { count, owner: bodyOwner } = body;
    if (!count || count <= 0) {
    ...
    const owner = bodyOwner || session?.value || 'office.sovereign.ag@gmail.com';

    const db = await readDB();
    const orgId = 'SVTP-org'; // Standard institutional scope
    ...
    // Institutional Debt Accrual: Proceed even if subsidy is exhausted.
    // The debt will be settled in the next billing cycle.
