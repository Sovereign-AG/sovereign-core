...
export default function IdentityForge() {
  ...
  useEffect(() => {
    ...
    if (authStatus === 'authenticated' && session?.user?.email && !did && !isMinting) {
      console.log(`[SOVEREIGN_OIDC] Authenticated as ${session.user.email}, bridging to registry...`);
  ...
  const handleAutoOnboard = async (userEmail: string) => {
    ...
    try {
      ...
      console.error('[SOVEREIGN_OIDC] Bridge Fault:', err);
    } finally {
      setIsMinting(false);
    }
  };
              ...
              {isMinting && authStatus === 'authenticated' ? (
                ...
                <div className="pt-4 text-center">
                  <p className="text-[10px] font-bold text-gray-700 uppercase tracking-widest leading-relaxed">
                    By initializing, you agree to the <Link href="/governance/rules" className="text-gray-500 hover:text-[#CBFF00]">Sovereign Rules</Link> and <Link href="/governance/privacy" className="text-gray-500 hover:text-[#CBFF00]">Privacy Protocol</Link>.
                  </p>
                </div>
              </form>
            )}
