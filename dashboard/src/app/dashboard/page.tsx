...
import {
  ...
  Stethoscope, CreditCard, Landmark, ReceiptText, Zap, ChevronRight, Calendar, ShieldCheck as ShieldCheckIcon
} from 'lucide-react';
...
const LiabilityLogo = ({ className = "" }) => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className={cn("text-gray-400", className)}>
    <path d="M 4 20 L 10 12 L 14 16 L 20 6" strokeLinecap="round" strokeLinejoin="round" />
    <circle cx="20" cy="6" r="2" fill="currentColor" />
  </svg>
);

const TrustScoreLogo = ({ className = "" }) => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className={cn("text-gray-400", className)}>
...
const LiabilityClockLogo = ({ className = "" }: { className?: string }) => (
  <div className={cn("relative flex items-center justify-center", className)}>
    {/* Outer Precision Ring */}
    <motion.div
      animate={{ rotate: 360 }}
      transition={{ duration: 8, repeat: Infinity, ease: "linear" }}
      className="absolute inset-0 border-[1.5px] border-indigo-500/20 rounded-full"
    />
    {/* Scanning Aperture */}
    <motion.div
      animate={{ rotate: 360 }}
      transition={{ duration: 2.5, repeat: Infinity, ease: "linear" }}
      className="absolute inset-0 border-[1.5px] border-indigo-400/40 rounded-full border-t-transparent border-r-transparent"
    />
    {/* Core Chrono Icon */}
    <div className="relative z-10 bg-zinc-950 p-0.5 rounded-full shadow-inner shadow-indigo-500/10">
      <Clock className="w-full h-full text-indigo-400 stroke-[2.5]" />
    </div>
  </div>
);
...
const SovereignLogo = ({ size = 24, className = "" }) => (
  <svg width={size} height={size} viewBox="0 0 100 100" className={className} fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    {/* Branded Identity Token - REMOVED per user request */}
    {/* <motion.circle
      cx="82"
      cy="35"
      r="4.5"
...
export default function App() {
  ...
  const [userRole, setUserRole] = useState('FLEET_MANAGER');
  const [userOrg, setUserOrg] = useState('sovereign-org');
  ...
  const [integrations, setIntegrations] = useState<any>({ mcp_bridge: { active: true, protocol_version: 'v1.2', alignment: 'NIST-CAISI' } });
  ...
  const [totalSignedActions, setTotalSignedActions] = useState(0);
  ...
  const [showAnchorSuccess, setShowAnchorSuccess] = useState(false);
  ...
  useEffect(() => {
    const checkAuth = async () => {
      let email = localStorage.getItem('svtp_anchor_email');

      if (!email) {
        const res = await fetch('/api/auth/svtp-session');
        const data = await res.json();
        if (data.active && data.email) {
          email = data.email;
          localStorage.setItem('svtp_anchor_email', email || '');
      ...
      fetch('/api/agent/list?limit=1000').then(r => r.json()).then(async data => {
        if (data.success) {
          setAgents(data.agents);
          
          let myAgent = data.agents.find((a: any) =>
            a.alias === 'ROOT_IDENTITY_ANCHOR' && 
          ...
          if (!myAgent && email) {
            console.log(`[SVTP_SYSTEM] No anchor found for ${email}. Initializing auto-provisioning...`);
          ...
          if (myAgent) {
            ...
            if (myAgent.owner && myAgent.owner !== 'SOVEREIGN FOUNDER') {
              localStorage.setItem('svtp_anchor_email', myAgent.owner);
              setUserEmail(myAgent.owner);
              
              // Resolve RBAC Profile
              fetch(`/api/user/profile?email=${encodeURIComponent(myAgent.owner)}`)
                .then(r => r.json())
                .then(profile => {
                  if (profile.success) {
                    setUserRole(profile.user.role);
                    setUserOrg(profile.user.org_id);
                  }
                });
            }
          }
          // Fetch API Keys
          fetch(`/api/keys/list?org_id=${userOrg}`).then(r => r.json()).then(keyData => {
            if (keyData.success) setApiKeys(keyData.keys);
          }).catch(() => {});

          // Fetch Shadow Registry
          fetch('/api/agent/scan').then(r => r.json()).then(scanData => {
            if (scanData.success) setDiscoveredAgents(scanData.found_shadow_agents || []);
          }).catch(() => {});
        }
        setIsIdentifying(false);
      }).catch(e => {
        console.error("Watchtower Sync Fault:", e);
        setIsIdentifying(false);
      });
    };
  ...
  const fetchStats = () => {
      fetch('/api/stats').then(r => r.json()).then(data => {
        if (data.success) {
          ...
          setMtta(data.mtta || 12.4);
          setProtocolMode(data.protocolMode || 'GROWTH');
          setTotalYield(data.totalRevenue || 0);
        setAvgTrustScore(data.avgTrustScore || 99.5);
        setAvgFairnessScore(data.avgFairnessScore || 99.0);
        setAvgLatency(data.avgLatency || 0);
        setTotalSignedActions(data.usageLedgerCount || 0);
      ...
      fetch('/api/governance/requests').then(r => r.json()).then(data => {
        if (data.success) {
            setApprovals(data.requests.map((r: any) => ({
                id: r.id,
                action: r.description,
                agent: 'SYNCING...', // Resolved in render
                agentId: r.agentId,
                risk: r.severity,
                time: 'Pending Review',
                isHighRisk: r.severity === 'HIGH' || r.severity === 'CRITICAL'
            })));
        }
      });
    };
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (data.totalRevenue !== undefined) {
          ...
          const unbilled = Number(data.unbilledAssessments) || 0;
          
          setTotalRevenue(totalRev);
          setUsageLedgerCount((Number(data.actions) || 0) + (Number(data.pulses) || 0));
          setTotalActiveAgents(Number(data.mints) || 0);
          setUnbilledAssessments(unbilled);
          
          // REALIZED = Total earned - what's still waiting to be settled
          setRealizedRevenue(Math.max(0, totalRev - unbilled));
          
          // LIABILITY MITIGATED: Institutional benchmark ($12.50 protection per $0.01 action tax)
          setLiabilityMitigated((Number(data.actions) || 0) * 12.5);

          // PROJECTED MONTHLY: Extrapolate from Billing Cycle Start
          if (data.billingCycleStart) {
            const start = new Date(data.billingCycleStart).getTime();
            const now = Date.now();
            const elapsedHours = (now - start) / (1000 * 60 * 60);
            
            if (elapsedHours > 0.01) { // Wait for a small amount of data
              const hourlyRate = totalRev / elapsedHours;
              setProjectedMonthlyRevenue(hourlyRate * 24 * 30);
            }
          }
          if (data.heartbeats) {
        ...
        console.error("[SVTP_STREAM] Malformed event data:", err);
      }
    };
  ...
  useEffect(() => {
    fetch('/api/agent/list?limit=10000').then(r => r.json()).then(data => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      // Increment mitigated risk value by $0.01 per second of institutional uptime
      setLiabilityMitigated(p => p + 0.01);
    }, 1000);
    return () => clearInterval(interval);
  }, []);
  ...
  const handleApproval = async (id: string, decision: 'APPROVED' | 'REJECTED') => {
    try {
      const res = await fetch('/api/governance/requests', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'PROCESS', requestId: id, decision })
      });
      if (res.ok) {
        setApprovals(p => p.filter(a => a.id !== id));
        setAuditLogs(p => [{ id: `log-${Date.now()}`, time: new Date().toISOString(), actor: userRole, action: `GOVERNANCE ${decision}`, target: id, status: 'Success', hash: `0x${Math.random().toString(16).slice(2, 10)}` }, ...p]);
        fetchStats();
      }
    } catch (e) { }
  };
  ...
  const handleDownloadGlobalLedger = async () => {
    const statsData = {
      avgTrustScore: agents.reduce((acc: number, curr: any) => acc + Number(curr.trust_index || 99.0), 0) / (agents.length || 1),
      avgFairnessScore: 99.4,
      totalVerifications: totalVerifications,
      balance: balance
    };
    await generateGlobalExecutiveLedger(statsData, agents);
  };
  ...
  const handleGenerateNodeKey = async () => {
    ...
    try {
      const res = await fetch('/api/keys/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ org_id: userOrg })
      });
      const data = await res.json();
      if (data.success) {
        setGeneratedApiKey(data.api_key);
        setShowAnchorSuccess(true);
        setTimeout(() => setShowAnchorSuccess(false), 5000);
        // Refresh local list
        const listRes = await fetch(`/api/keys/list?org_id=${userOrg}`);
  ...
  const handleBulkMint = async () => {
    if (provisionCount <= 0) return;
    
    setIsScanning(true); 
    try {
      const res = await fetch('/api/agent/provision/bulk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          count: provisionCount,
          baseAlias: 'INSTITUTIONAL_NODE',
          purpose: 'Auto-Provisioned Institutional Fleet Expansion'
        })
      });
      ...
      if (data.success) {
        // Refresh local fleet
        const listRes = await fetch('/api/agent/list?limit=1000');
        const listData = await listRes.json();
        if (listData.success) setAgents(listData.agents);
        
        // Update billing metrics
        setUnbilledAssessments(data.totalUnbilled);
        setProvisionCount(0);
      ...
      alert("Institutional Provisioning Engine Offline.");
    }
    setIsScanning(false);
    fetchStats();
  };
  ...
  const generateVaultKey = async () => {
    try {
      ...
      const res = await fetch('/api/governance/keys', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ alias: aliasSelection, org_id: userOrg })
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleTopUp = async () => {
    const amountStr = prompt("Enter Funding Amount (USD):", "100.00");
    if (!amountStr) return;
    ...
    try {
      const res = await fetch('/api/billing/topup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ amount, orgId: 'sovereign-org' })
      });
      const data = await res.json();
      if (data.success) {
        setBalance(data.newBalance);
        setAuditLogs(p => [{ id: `log-${Date.now()}`, time: new Date().toISOString(), actor: userRole, action: `FUNDING_DEPOSIT`, target: `+$${amount}`, status: 'Success', hash: `0x${Math.random().toString(16).slice(2, 10)}` }, ...p] as any);
      }
    } catch (e) {}
    setIsUpgrading(false);
  };

  const handleExportIdentityLedger = () => {
    const data = agents.map(a => ({
      alias: a.alias,
      did: a.did,
      public_key: a.public_key,
      trust_index: a.trust_index,
      status: a.status,
      nist_compliance: a.nist || 'Verified'
    }));
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `svtp_identity_ledger_${Date.now()}.json`;
    a.click();
  ...
  const executePurge = async () => {
    ...
    try {
      const res = await fetch('/api/governance/purge', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ org_id: userOrg, confirm_delete: true })
      });
      if (res.ok) {
        alert("SVTP Destruction Protocol Complete. Registry entries sanitized.");
  ...
  const exportIdentityLedger = async () => {
    try {
      ...
      const dlAnchorElem = document.createElement('a');
      dlAnchorElem.setAttribute("href", dataStr);
      dlAnchorElem.setAttribute("download", `svtp_identity_ledger_${Date.now()}.json`);
      dlAnchorElem.click();
    } catch (e) { }
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
        ...
        <div className="bg-[#0A0A0A] border border-[#222] rounded-none p-6 font-mono text-xs overflow-hidden relative group">
          ...
          <pre className="text-gray-400 space-y-1">
            <div className="text-emerald-500">// SVTP Python Handshake</div>
            <div><span className="text-indigo-400">from</span> svtp <span className="text-indigo-400">import</span> Agent</div>
            <br />
            ...
            <br />
            <div className="text-gray-500">// Status: did:svtp:vj0sx6ah... [Registry Anchored]</div>
          </pre>
        </div>
      </div>
    </motion.div>
  ...
  const renderDeveloperForge = () => (
    <motion.div key="forge" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="w-full space-y-6">
      {activeView === 'installation' ? (
        <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl overflow-hidden shadow-sm">
          ...
          <div className="p-8 space-y-6">
            <p className="text-gray-400 text-sm leading-relaxed">
              Deploy the SVTP identity anchor to your local or cloud infrastructure. This client handles Ed25519 handshakes and real-time telemetry pulses.
            </p>
            <div className="space-y-4">
              ...
              <div className="flex bg-white/[0.03] border border-white/10 rounded-xl p-4 font-mono text-[13px] text-emerald-400 justify-between items-center group shadow-sm">
                <code className="break-all whitespace-pre-wrap select-all">
                  powershell -Command "irm http://{typeof window !== 'undefined' ? window.location.host : 'localhost:3001'}/api/quickstart | python"
                </code>
                <button 
                  onClick={() => { 
                    const host = typeof window !== 'undefined' ? window.location.host : 'localhost:3001';
                    const protocol = host.includes('localhost') ? 'http' : 'https';
                    navigator.clipboard.writeText(`powershell -Command "irm ${protocol}://${host}/api/quickstart | python"`);
                    setCopiedKey('quickstart');
                    setTimeout(() => setCopiedKey(null), 2000);
                  }} 
                  className="ml-4 shrink-0 p-2 bg-white/5 border border-white/10 rounded-xl text-gray-400 hover:text-white hover:bg-white/10 transition-colors shadow-sm"
            ...
            <div className="space-y-3 pt-4 border-t border-white/10">
              ...
              <div className="flex bg-white/[0.03] border border-white/10 rounded-xl p-4 font-mono text-[13px] text-gray-300 justify-between items-center group shadow-sm">
                <span className="flex items-center">
                  <span className="text-gray-500 mr-2">$</span> pip install svtp-sdk
                </span>
                <button 
                  onClick={() => { 
                    navigator.clipboard.writeText('pip install svtp-sdk');
                    setCopiedKey('pip');
                    setTimeout(() => setCopiedKey(null), 2000);
                  }} 
                  className="p-2 bg-white/5 border border-white/10 rounded-xl text-gray-400 hover:text-white hover:bg-white/10 transition-colors shadow-sm"
              ...
              <div className="flex items-center">
                ...
                {showAnchorSuccess && (
                  <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="ml-4">
                    <Badge type="Verified">Key Generated</Badge>
                  </motion.div>
                )}
              </div>
            ...
            {apiKeys.length > 0 ? (
              <div className="space-y-4">
                {apiKeys.map((key: any, i: number) => (
                  <div key={key.id || `node-key-${i}`} className="p-4 border border-white/10 rounded-2xl flex justify-between items-center bg-white/[0.02] group hover:bg-white/[0.04] transition-all cursor-pointer">
              ...
              <div className="flex flex-col items-center justify-center py-12 border-2 border-dashed border-white/10 rounded-3xl bg-white/[0.02]">
                <div className="p-5 bg-white/5 rounded-xl mb-6 relative">
        ...
        <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl overflow-hidden shadow-sm">
          ...
          <div className="p-8 space-y-6">
            <p className="text-gray-400 text-sm leading-relaxed">
              Generate long-lived tokens for CI/CD pipelines and backend automation. All integration tokens are logged under the <span className="text-white font-bold">SVTP Audit Trail</span>.
            </p>
            <div className="space-y-3">
              ...
              <div className="flex bg-white/[0.03] border border-white/10 rounded-xl p-4 justify-between items-center group shadow-sm">
                <span className="font-mono text-[13px] text-gray-300 select-all">{generatedApiKey || 'svtp_live_••••••••••••••••'}</span>
                <div className="flex space-x-2">
                  {generatedApiKey && (
                    <button onClick={() => { navigator.clipboard.writeText(generatedApiKey); setCopiedKey('gen_key'); setTimeout(() => setCopiedKey(null), 2000); }} className="p-2.5 bg-white/5 border border-white/10 rounded-xl text-gray-400 hover:text-white hover:bg-white/10 transition-colors">
                  ...
                  <button onClick={async () => {
                    try {
                      setIsUpgrading(true);
                      const res = await fetch('/api/keys/generate', { method: 'POST', body: JSON.stringify({ org_id: userOrg }) });
                      const data = await res.json();
                      if (data.success) {
                        setGeneratedApiKey(data.api_key);
                        setShowAnchorSuccess(true);
            ...
            <div className="pt-6 border-t border-white/10 mt-6 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3">
                   <Badge type={integrations.mcp_bridge?.active ? "Active" : "Low"}>
                     {integrations.mcp_bridge?.active ? "FEDERATED_ACTIVE" : "BRIDGE_OFFLINE"}
                   </Badge>
                   <button
                     onClick={() => updateSettings({ integrations: { mcp_bridge: { ...integrations.mcp_bridge, active: !integrations.mcp_bridge?.active } } })}
                     className={`w-10 h-5 rounded-full relative transition-colors ${integrations.mcp_bridge?.active ? 'bg-lime-500 shadow-[0_0_10px_rgba(132,204,22,0.3)]' : 'bg-gray-700'}`}
                   >
                     <div className={`absolute top-1 w-3 h-3 rounded-full transition-all shadow-sm ${integrations.mcp_bridge?.active ? 'bg-white right-1' : 'bg-gray-400 left-1'}`} />
                   </button>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-5 bg-white/[0.02] border border-white/10 rounded-2xl">
                  ...
                  <div className="flex items-center space-x-2">
                    <div className={cn("w-2 h-2 rounded-full shadow-[0_0_8px_rgba(132,204,22,0.5)]", integrations.mcp_bridge?.active ? "bg-lime-500 animate-pulse" : "bg-gray-600")} />
                    <span className="text-[13px] text-white font-bold font-mono">
                      {integrations.mcp_bridge?.active ? "SYNCED" : "UNINITIALIZED"}
                    </span>
                  </div>
                  <p className="text-xs text-gray-500 mt-3 leading-relaxed font-medium">Bridge status between Internal Data Silos and External Workflows.</p>
                </div>
                <div className="p-5 bg-white/[0.02] border border-white/10 rounded-2xl">
                  <div className="text-xs font-semibold text-gray-400 mb-2">Protocol Alignment</div>
                  <div className="text-[13px] text-white font-bold flex items-center">
                    <ShieldCheck size={16} className="mr-2 text-lime-500" /> {integrations.mcp_bridge?.alignment || 'NIST-CAISI'}
                  </div>
                  <p className="text-xs text-gray-500 mt-3 leading-relaxed font-medium">Model Context Protocol {integrations.mcp_bridge?.protocol_version || 'v1.2'} implemented for identity security.</p>
                </div>
              </div>
            </div>
          </div>
  ...
  const renderDashboard = () => (
    <motion.div key="dash" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6 gap-4">
        {[
          { title: 'Fleet Integrity Score', val: `${fleetIntegrity.toFixed(1)}%`, sub: 'Sovereign Green Pulse', Icon: ShieldCheck, color: '#10B981', bg: 'rgba(16, 185, 129, 0.1)' },
          { title: 'Institutional Liability Clock', val: formatDuration(liabilityClock), sub: 'NIST-2026 Uptime Alignment', Icon: LiabilityClockLogo, color: '#6366F1', bg: 'rgba(99, 102, 241, 0.1)' },
          { title: 'Carbon Intensity', val: `${(carbonIntensity * 1000).toFixed(2)}`, sub: 'Metric: gCO2 / Pulse', Icon: Globe, color: '#14B8A6', bg: 'rgba(20, 184, 166, 0.1)' },
          { title: 'Pulse Metabolism', val: `${pulseRate.toFixed(1)}/s`, sub: 'Global Topology Pulses', Icon: Activity, color: '#F43F5E', bg: 'rgba(244, 63, 94, 0.1)' },
          { title: 'Handshake Latency', val: `${avgLatency > 0 ? avgLatency.toFixed(2) : '---'} ms`, sub: 'Institutional Performance', Icon: Zap, color: '#F59E0B', bg: 'rgba(245, 158, 11, 0.1)' },
          { title: 'Integrity Signatures', val: totalSignedActions.toLocaleString(), sub: 'HMAC-SHA256 Verified', Icon: ShieldCheck, color: '#3B82F6', bg: 'rgba(59, 130, 246, 0.1)' },
          {
          ...
          <div key={item.title} title={(item as any).tooltip} className="bg-white/5 backdrop-blur-md border border-white/5 p-5 rounded-3xl flex flex-col justify-between h-36 relative overflow-hidden group hover:bg-white/[0.08] transition-all cursor-help shadow-sm">
            <div className="flex justify-between items-start text-gray-500 relative z-10">
              ...
              <div className="p-1.5 rounded-lg" style={{ backgroundColor: item.bg }}>
            ...
            {item.title === 'Institutional Liability Clock' && (
               <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 bg-indigo-500/5 blur-[40px] rounded-full pointer-events-none" />
            )}
            <Sparkline />
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-zinc-950 border border-zinc-900 p-6 rounded-3xl shadow-sm relative overflow-hidden group hover:bg-zinc-900/50 transition-all h-full flex flex-col justify-between">
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/5 p-6 rounded-3xl shadow-sm relative overflow-hidden group transition-all h-full flex flex-col justify-between">
          <div className="flex justify-between items-start mb-4 relative z-10">
            <h3 className="text-[11px] font-medium tracking-wide text-gray-400 flex items-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-zinc-950 border border-zinc-900 p-6 rounded-3xl shadow-sm relative overflow-hidden group transition-all">
          <div className="flex justify-between items-start mb-4 relative z-10">
            <h3 className="text-[11px] font-medium tracking-wide text-gray-400 flex items-center">
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/5 p-6 rounded-3xl shadow-sm relative overflow-hidden group transition-all">
          <div className="flex justify-between items-start mb-4 relative z-10">
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/5 p-6 rounded-3xl shadow-sm relative overflow-hidden group transition-all">
          <div className="flex justify-between items-start mb-4 relative z-10">
            <h3 className="text-[11px] font-medium tracking-wide text-gray-400 flex items-center relative z-10">
      ...
      <div className="flex justify-between items-center bg-zinc-950 border border-zinc-900 rounded-3xl p-6 shadow-2xl">
        <div>
        ...
        <div className="flex space-x-3">
          ...
          <button 
            onClick={() => generateGlobalExecutiveLedger({
              avgTrustScore: fleetIntegrity,
              avgFairnessScore: fairnessScore,
              totalVerifications: totalVerifications,
              balance: balance
            }, agents)}
      ...
      {discoveredAgents && discoveredAgents.length > 0 ? (
        <div className="bg-red-500/[0.02] border border-red-500/10 rounded-[32px] p-8 mt-8 relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-8 opacity-[0.03] text-red-500 pointer-events-none group-hover:opacity-[0.05] transition-opacity">
            <ShieldAlert size={120} />
          </div>
          <div className="flex justify-between items-center mb-8 relative z-10">
            <div>
              <h3 className="text-[10px] font-black text-red-500 uppercase tracking-[0.25em] mb-1 flex items-center">
                <div className="w-1.5 h-1.5 bg-red-500 rounded-full mr-2 animate-pulse" />
                Shadow AI Infrastructure Detected
              </h3>
              <p className="text-sm text-gray-400 font-medium tracking-tight">Unauthorized Localhost Endpoints identified in local topology.</p>
            </div>
            <button 
              onClick={() => handleGlobalKillSwitch()}
              className="px-4 py-2 bg-red-500/10 border border-red-500/20 text-red-500 text-[10px] font-bold uppercase tracking-widest rounded-xl hover:bg-red-500 hover:text-white transition-all"
            >
              Neutralize All
            </button>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 relative z-10">
            {discoveredAgents.map((app: any, i: number) => (
              <div key={i} className="flex items-center justify-between p-6 bg-zinc-950/50 border border-white/5 rounded-[24px] group/item hover:border-red-500/30 hover:bg-zinc-900/50 transition-all shadow-sm">
                <div className="flex items-center space-x-5">
                  <div className="p-3 bg-red-500/5 border border-red-500/10 rounded-2xl text-red-500 group-hover/item:scale-105 transition-transform">
                    <ScanSearch size={20} className="animate-spin-slow" />
                  </div>
                  <div>
                    <div className="text-base font-bold text-white tracking-tight leading-none mb-1.5">{app.alias}</div>
                    <div className="flex items-center space-x-2">
                       <span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Type: {app.type || 'Inference Node'}</span>
                       <span className="w-1 h-1 bg-gray-700 rounded-full" />
                       <span className="text-[10px] text-emerald-500 font-bold uppercase tracking-widest">Traffic: Active</span>
                    </div>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
                  <div className="text-right hidden sm:block">
                     <div className="px-2 py-1 bg-red-500/10 text-red-500 text-[8px] font-black border border-red-500/20 rounded-full uppercase tracking-tighter">High Risk Signature</div>
                  </div>
                  <button 
                    onClick={() => {
                      setProvisionAlias(app.alias);
                      setProvisionPurpose(`Capture: ${app.alias}`);
          ...
          <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-between">
             <div className="text-[9px] text-gray-600 font-mono uppercase tracking-widest">NIST-SP-800-218 Enforcement Active</div>
             <div className="text-[9px] text-gray-600 font-mono uppercase tracking-widest">Registry Sync: PENDING</div>
          </div>
        </div>
      ) : hasScanned && (
        <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-3xl p-6 mt-6 flex items-center justify-between">
           <div className="flex items-center space-x-4">
           ...
           <div className="px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 text-[10px] text-emerald-500 font-bold uppercase tracking-widest rounded-xl">
              Verified Secure
           </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white/5 backdrop-blur-md border border-white/5 p-6 rounded-3xl relative overflow-hidden min-h-[300px] flex flex-col shadow-sm">
          ...
          <div className="absolute inset-0 flex items-center justify-center p-8 opacity-30 pointer-events-none">
            <div className="flex w-full items-center justify-between text-xs font-mono text-gray-500">
              <div className="flex flex-col items-center">
                <div className="h-2 w-2 bg-emerald-500 rounded-full mb-2 shadow-[0_0_8px_rgba(16,185,129,0.4)]"></div>
                <span className="bg-[#050505] px-2 relative z-10">PROTOCOL NODE</span>
              </div>
              <div className="flex-1 border-t border-dashed border-white/10 mx-2 relative">
                <div className="absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 px-3 bg-[#050505] text-[10px] tracking-widest text-gray-400">SECURE TUNNEL</div>
              </div>
              <div className="flex flex-col items-center">
                <div className="h-2 w-2 bg-gray-800 rounded-full mb-2"></div>
                <span className="bg-[#050505] px-2 relative z-10">ROOT</span>
              </div>
            </div>
          </div>
          ...
          <div className="relative z-10 w-full flex-1 flex flex-col justify-end mt-4">
            ...
            <div className="space-y-1.5 overflow-hidden h-32 flex flex-col justify-end pb-2">
              {heartbeats.map((log, i) => (
                <motion.div key={log} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1 - (i * 0.2), x: 0 }} className="text-xs font-mono truncate" style={{ color: i === 0 ? '#34d399' : '#6b7280' }}>
                  {log}
                </motion.div>
              ))}
            </div>
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/5 p-6 rounded-3xl shadow-sm">
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/5 p-6 rounded-3xl shadow-sm relative overflow-hidden group">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center bg-black/40 border border-[#1a1a1a] p-3 rounded-xl">
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/5 p-6 rounded-3xl shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-sm font-medium text-white">SVTP Compliance Logs</h3>
            <button 
              onClick={() => {
                const stats = {
                  avgTrustScore,
                ...
                generateGlobalExecutiveLedger(stats, agents);
              }} 
              className="text-[10px] font-mono uppercase tracking-widest text-emerald-400 hover:text-white flex items-center transition-colors"
            >
              <Shield size={12} className="mr-2" /> Institutional Report
            </button>
          </div>
          <div className="space-y-4">
            {auditLogs.slice(0, 4).map((log, index) => (
              <div key={log.id || `audit-${index}`} className="flex flex-col pb-3 border-b border-[#222] last:border-0 last:pb-0">
    ...
    <motion.div key="treasury" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          <div key={stat.title} className="bg-zinc-950 border border-zinc-900 p-8 rounded-none shadow-xl group hover:bg-zinc-900/50 transition-all relative overflow-hidden">
            ...
            <div className="mt-6 relative z-10">
              <div className={`w-full h-1 rounded-full overflow-hidden ${stat.barBg}`}>
                <motion.div
                  ...
                  animate={{ 
                    width: `${
                      i === 0 ? Math.min(100, (realizedRevenue / (totalRevenue || 1)) * 100) :
                      i === 1 ? Math.min(100, (totalRevenue / (settlementThreshold || 1000)) * 100) :
                      Math.min(100, (projectedMonthlyRevenue / 5000) * 100)
                    }%` 
                  }}
      ...
      <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-none p-8 shadow-2xl overflow-hidden relative group">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
          {[
            ...
            <div key={price.label} className="p-6 border border-zinc-900 bg-zinc-950 rounded-none hover:bg-zinc-900/50 transition-colors shadow-sm">
        ...
        <div className="mt-8 pt-8 border-t border-white/10 relative z-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
            ...
            <div className="flex space-x-2">
              <span className="px-2 py-1 bg-white/10 text-gray-300 text-[10px] font-semibold rounded-lg border border-white/5">x402-USDC Enabled</span>
              <span className="px-2 py-1 bg-white/10 text-gray-300 text-[10px] font-semibold rounded-lg border border-white/5">Protocol v1.0.4-E</span>
            </div>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
             <div className="lg:col-span-3 p-6 bg-zinc-950 border border-zinc-900 rounded-3xl flex items-center justify-between shadow-sm">
                <div className="flex items-center space-x-4">
                ...
                <div className="text-right">
                  ...
                  <div className={`text-[11px] font-semibold px-2 py-1 rounded-full inline-block ${unbilledAssessments < (settlementThreshold || 1000) * 0.9 ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'}`}>
                    {unbilledAssessments < (settlementThreshold || 1000) * 0.9 ? 'Optimal' : 'Pending'}
                  </div>
                </div>
             </div>
             <div className="p-6 bg-zinc-950 border border-zinc-900 rounded-3xl text-center flex flex-col justify-center shadow-sm">
                <div className="text-[11px] text-gray-500 font-medium mb-1">Total Settled</div>
                <div className="text-xl font-semibold text-white">
                  <AnimatedNumber value={realizedRevenue} />
                </div>
             </div>
          </div>
        </div>
    ...
    return (
      <motion.div key="app" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-4">
        {combinedApprovals.length === 0 ? (
          <div className="p-12 text-center text-gray-500 border border-dashed border-[#333] rounded-3xl bg-[#0A0A0A]">Zero pending compliance requests.</div>
        ) : combinedApprovals.map((app: any) => (
          <div key={app.id} className="bg-[#0A0A0A] border border-[#222] rounded-3xl p-6 flex justify-between items-center group hover:border-[#333] transition-all">
            <div className="flex items-center space-x-6">
              <div className={cn("p-3 rounded-xl", app.isHighRisk ? "bg-red-500/10 text-red-500 shadow-[0_0_15px_rgba(239,68,68,0.2)]" : "bg-white/5 text-gray-500")}>
                <ShieldAlert size={20} />
              </div>
              <div>
                <div className="flex items-center space-x-3 mb-1">
                  {app.isHighRisk && <span className="px-1.5 py-0.5 bg-red-600/20 text-red-500 text-[7px] font-black border border-red-500/30 rounded-full uppercase tracking-widest animate-pulse">High Risk Signature</span>}
                ...
                <p className="text-xs text-gray-400 mt-0.5 font-medium">SVTP Agent: {agents.find(a => a.id === app.agentId || a.did === app.agentId)?.alias || app.agent}</p>
              </div>
            </div>
            <div className="flex space-x-2">
              <button onClick={() => handleApproval(app.id, 'REJECTED')} className="px-4 py-2 rounded-xl bg-[#0A0A0A] border border-[#222] text-gray-400 hover:text-white hover:bg-[#111] text-[10px] font-bold uppercase tracking-widest transition-all">Deny</button>
              <button onClick={() => handleApproval(app.id, 'APPROVED')} className="px-5 py-2 rounded-xl bg-white text-black hover:bg-gray-200 text-[10px] font-bold uppercase tracking-widest transition-all shadow-[0_0_20px_rgba(255,255,255,0.1)]">Authorize Action</button>
            </div>
          </div>
        ))}
      </motion.div>
  ...
  const renderBilling = () => (
    <motion.div key="billing" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-zinc-950 border border-zinc-900 p-8 rounded-3xl shadow-sm relative overflow-hidden group hover:bg-zinc-900/50 transition-all space-y-8">
          <div className="absolute top-0 right-0 p-8 opacity-[0.03] text-white group-hover:opacity-[0.05] transition-opacity">
            <DollarSign size={160} />
          </div>
          <div className="relative z-10">
          ...
          <div className="pt-8 border-t border-white/10 space-y-6 relative z-10">
             ...
             <div className="flex justify-between items-center text-xs font-medium">
                ...
                <div className="flex items-center space-x-3">
                   <span className="text-emerald-500 font-bold"><AnimatedNumber value={balance} /></span>
                   <button 
                     onClick={handleTopUp}
                     className="px-2 py-0.5 bg-emerald-500/10 text-emerald-500 text-[9px] font-black border border-emerald-500/20 rounded-lg hover:bg-emerald-500 hover:text-white transition-all uppercase tracking-widest"
                   >
                     Top Up
                   </button>
                </div>
        ...
        <div className="bg-zinc-950 border border-zinc-900 p-8 rounded-3xl shadow-sm space-y-6">
          <h3 className="text-sm font-semibold text-white mb-2 flex items-center">
          ...
          <div className="space-y-4">
             {[
               ...
               <div key={rate.label} className="flex justify-between items-center p-5 bg-zinc-950 border border-zinc-900 rounded-2xl group hover:bg-zinc-900/50 transition-all">
                  <div>
      ...
      <div className="bg-zinc-950 border border-zinc-900 p-8 rounded-3xl shadow-sm">
         ...
         <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
            {auditLogs.filter(log => log.action.includes('Tax') || log.action.includes('Deduction')).slice(0, 10).map((log) => (
              <div key={log.id} className="flex justify-between items-center p-4 border border-zinc-900 bg-zinc-950 rounded-xl mb-2 hover:bg-zinc-900/50 transition-colors">
  ...
  const renderIdentityVault = () => (
    <motion.div key="vault" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-6 w-full">
      <section className="bg-white/5 backdrop-blur-3xl border border-white/10 rounded-3xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
        ...
        <div className="px-6 py-4 border-b border-white/10 bg-white/[0.01]">
          <div className="relative group">
            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-emerald-500 transition-all duration-300">
              <Search size={18} />
            </div>
            <input 
              type="text" 
              placeholder="Filter identity anchors by alias or public key..." 
              value={vaultSearch}
              onChange={(e) => setVaultSearch(e.target.value)}
              className="w-full bg-black/60 border border-white/5 hover:border-white/10 focus:border-emerald-500/50 py-3.5 pl-14 pr-4 text-[13px] text-white placeholder-gray-600 focus:outline-none transition-all rounded-2xl font-mono shadow-inner shadow-black/50"
        ...
        <div className="p-6 space-y-4 max-h-[500px] overflow-y-auto custom-scrollbar">
          {vaultKeys.filter(k => 
            k.alias.toLowerCase().includes(vaultSearch.toLowerCase()) || 
            k.key.toLowerCase().includes(vaultSearch.toLowerCase())
          ).length > 0 ? vaultKeys.filter(k => 
            k.alias.toLowerCase().includes(vaultSearch.toLowerCase()) || 
            k.key.toLowerCase().includes(vaultSearch.toLowerCase())
          ).map((key, index) => (
            <motion.div 
              key={key.id || `vault-key-${index}`} 
              whileHover={{ y: -4, backgroundColor: 'rgba(255, 255, 255, 0.04)', borderColor: 'rgba(255, 255, 255, 0.2)' }}
              className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/10 rounded-2xl group transition-all duration-300 shadow-lg hover:shadow-emerald-500/5"
            >
              <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-4">
                ...
                <button 
                  ...
                  className="p-2 text-gray-500 hover:text-emerald-400 hover:bg-emerald-500/10 rounded-xl transition-all duration-200"
                  title="Copy Key"
                >
                  {copiedKey === key.id ? <Check size={16} /> : <Copy size={16} />}
                </button>
                <button 
                  onClick={() => revokeVaultKey(key.id)} 
                  className="p-2 text-gray-500 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all duration-200 active:scale-90 group/trash border border-transparent hover:border-red-500/20 shadow-sm" 
                  title="Revoke Key"
                >
                  <Trash2 size={16} className="transition-colors duration-200 group-hover/trash:text-red-500 group-active/trash:scale-90" />
                </button>
              </div>
            </motion.div>
          )) : (
            <div className="py-12 flex flex-col items-center justify-center border-2 border-dashed border-white/10 rounded-3xl bg-white/[0.02]">
              <KeyRound className="text-gray-400 w-8 h-8 mb-4 opacity-50" />
              <div className="text-[13px] text-gray-400 font-medium text-center">
                {vaultSearch ? (
                  <>
                    <div className="text-white mb-1">No Anchors Match Your Search</div>
                    <div className="text-[11px] opacity-60">Try searching for a different alias or partial key</div>
                  </>
                ) : "No Active Identity Anchors Detected"}
              </div>
            </div>
          )}
        ...
        <div className="px-6 py-5 border-t border-white/10 bg-white/[0.02] flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className={`p-2.5 rounded-xl flex items-center justify-center transition-colors ${governance.tfa_active_for_revocation ? 'bg-emerald-500/10 text-emerald-500' : 'bg-white/5 text-gray-500'}`}>
              <Lock size={16} />
            </div>
            <div>
              <span className="text-[13px] font-bold text-gray-200 block tracking-tight">Enforce MFA for Revocation</span>
              <span className="text-[10px] text-gray-500 uppercase tracking-[0.2em] font-mono font-medium">Institutional Protocol 08-B</span>
            </div>
          </div>
          <button
            onClick={() => updateSettings({ governance: { tfa_active_for_revocation: !governance.tfa_active_for_revocation } })}
            className={`w-12 h-6 rounded-full relative transition-all duration-300 shadow-inner flex items-center px-1 ${governance.tfa_active_for_revocation ? 'bg-emerald-500' : 'bg-zinc-800'}`}
          >
            <motion.div 
              initial={false}
              animate={{ 
                x: governance.tfa_active_for_revocation ? 24 : 0
              }}
  ...
  const renderGovernanceSettings = () => (
    <motion.div key="gov" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-6 w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <section className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl p-8 shadow-sm">
          ...
          <div className="space-y-6">
            ...
            <button 
               onClick={handleExportIdentityLedger}
               className="w-full py-2.5 bg-white/5 border border-white/10 text-gray-300 rounded-xl hover:bg-white/10 hover:text-white transition-colors text-[13px] font-semibold flex items-center justify-center"
             >
            ...
            <div>
              ...
              <div className="flex items-center space-x-4 bg-white/[0.03] border border-white/10 rounded-xl p-2.5">
                <input
                  ...
                  className="w-8 h-8 rounded-xl bg-transparent border-0 cursor-pointer overflow-hidden p-0"
                />
      ...
      <section className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-sm">
        <h3 className="text-[13px] font-bold text-gray-300 uppercase tracking-widest mb-6 flex items-center">
      ...
      <section className="bg-[#050505] border border-emerald-500/30 p-8 rounded-3xl relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
           <div className="p-10 border border-white/10 bg-white/[0.02] rounded-3xl flex flex-col items-center justify-center space-y-6 relative group cursor-pointer shadow-sm hover:border-emerald-500/30 hover:bg-white/[0.04] transition-all">
           ...
           <div className="space-y-6">
              <div className="p-6 bg-white/5 border border-white/10 rounded-2xl">
                 ...
                 <div className="bg-white/[0.02] border border-white/5 p-4 rounded-2xl font-mono text-[11px] text-gray-400 break-all leading-relaxed h-32 overflow-y-auto custom-scrollbar">
  ...
  <text x="50" y="65" fill="gray" font-family="monospace" font-size="4" text-anchor="middle">NIST-2026</text>
                 ...
                 <div className="grid grid-cols-2 gap-3 mt-4">
                    <button 
                      ...
                      className="py-3 bg-white/5 border border-white/10 text-white text-[10px] font-bold rounded-2xl hover:bg-white/10 transition-all flex items-center justify-center"
                    >
                       {copiedBadge ? <Check size={14} className="mr-2 text-emerald-500" /> : <Copy size={14} className="mr-2" />}
                       {copiedBadge ? "Snippet Cached" : "Copy HTML"}
                    </button>
                    <button 
                      onClick={() => {
                        const svg = `<svg width="100" height="100" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
                        ...
                        const a = document.createElement('a');
                        a.href = url;
                        a.download = 'sovereign_verified_badge.svg';
                        a.click();
                        URL.revokeObjectURL(url);
                      }}
                      className="py-3 bg-emerald-600 text-white text-[10px] font-bold rounded-2xl hover:bg-emerald-500 transition-all flex items-center justify-center shadow-lg shadow-emerald-900/20"
                    >
      ...
      <section className="bg-red-500/5 border border-red-500/20 rounded-3xl p-10 flex flex-col items-center text-center shadow-sm relative overflow-hidden">
      ...
      {discoveredAgents.length > 0 ? (
        <div className="space-y-4 max-h-[420px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
          {discoveredAgents.map((actor: any) => (
            <div key={actor.did} className="bg-red-500/5 border border-red-500/10 p-4 rounded-2xl flex justify-between items-center group animate-in fade-in slide-in-from-right-4 hover:bg-red-500/10 transition-colors">
              <div className="flex items-center space-x-4">
                <div className="p-2 bg-red-500/10 rounded-xl text-red-500 uppercase font-black text-xs">
              ...
              <div className="flex space-x-2">
                <button
                  ...
                  className="px-4 py-2 bg-emerald-600 text-white text-[10px] font-black rounded-xl hover:bg-emerald-500 transition-all uppercase tracking-widest shadow-lg shadow-emerald-900/20"
                >
                  ONBOARD
                </button>
                <button
                  onClick={() => handleHunterAction(actor.did, 'KILL')}
                  className="px-4 py-2 bg-red-600/20 text-red-500 text-[10px] font-black rounded-xl hover:bg-red-600 hover:text-white transition-all border border-red-500/30 uppercase tracking-widest"
                >
                  KILL-SWITCH BLOCK
                </button>
              </div>
        ...
        <div className="text-center py-4 border border-dashed border-[#222] rounded-3xl bg-black/20 flex flex-col items-center justify-center space-y-2">
  ...
  const renderRegistry = () => (
    <motion.div key="reg" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl p-6 shadow-2xl gap-6">
        <div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          {lastPassports ? (
            <button onClick={downloadPassports} className="px-4 py-2 bg-emerald-600 text-white font-black rounded-xl text-[10px] hover:bg-emerald-500 transition-all uppercase tracking-widest flex items-center justify-center shadow-[0_0_20px_rgba(16,185,129,0.2)]">
              <Download size={14} className="mr-2" /> Download Fleet Passports (.JSON)
            </button>
          ) : (
            <div className="flex items-center bg-black/40 border border-white/10 rounded-xl overflow-hidden h-10">
              <div className="px-3 border-r border-white/10 flex flex-col justify-center h-full bg-white/[0.02]">
                <span className="text-[8px] font-black text-gray-500 uppercase tracking-tighter leading-none mb-0.5">Subsidy</span>
                <span className={cn("text-[9px] font-mono font-bold leading-none", freeSlots > 0 ? "text-emerald-500" : "text-amber-500")}>
                  {freeSlots > 0 ? `${freeSlots} REMAINING` : "LIMIT REACHED"}
                </span>
              </div>
              <input
                type="number"
                ...
                onChange={(e) => {
                  ...
                  if (isNaN(val)) setProvisionCount(0);
                  else setProvisionCount(Math.max(0, val));
                }}
                className="w-12 bg-transparent px-2 text-xs font-mono text-white focus:outline-none border-r border-white/10 h-full text-center"
              />
              <button 
                onClick={handleBulkMint} 
                disabled={isScanning || provisionCount === 0} 
                className="px-4 h-full bg-white text-black font-black text-[10px] uppercase tracking-widest hover:bg-gray-200 disabled:opacity-50 transition-colors flex items-center"
              >
                <Cpu size={12} className="mr-2" /> Bulk Provision
              </button>
            </div>
          )}
          <button 
            onClick={handleScan} 
            disabled={isScanning} 
            className="h-10 px-6 bg-indigo-600/10 border border-indigo-500/50 text-indigo-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600/20 transition-all flex items-center justify-center"
          >
            <ScanSearch size={14} className="mr-2" /> {isScanning ? 'Scanning...' : 'Analyze Network'}
          </button>
        </div>
      </div>
      ...
      <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl">
         <div className="p-4 border-b border-white/5 flex items-center bg-[#0d0d0d]">
            <div className="w-[200px] flex items-center space-x-2 shrink-0">
            ...
            <div className="flex-1 flex justify-center">
              <div className="relative group w-full max-w-[420px]">
               <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-emerald-500 transition-colors z-10" />
               <input 
                 type="text"
                 placeholder="SEARCH FLEET..."
                 value={searchTerm}
                 onChange={(e) => setSearchTerm(e.target.value)}
                 className="w-full pl-14 pr-4 py-3 bg-black/60 border border-white/10 rounded-2xl text-[11px] font-bold uppercase tracking-widest text-white focus:outline-none focus:border-emerald-500/50 transition-all shadow-2xl placeholder:text-gray-700 font-mono"
               />
              </div>
            </div>

            <div className="w-[200px] flex justify-end shrink-0">
              {(userRole === 'SUPER_ADMIN' || userRole === 'FLEET_MANAGER') && (
               <button 
                 onClick={handleDownloadGlobalLedger}
                 className="h-10 px-6 bg-emerald-500/10 border border-emerald-500/30 text-emerald-500 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-500/20 transition-all flex items-center shadow-lg whitespace-nowrap"
               >
                 <FileCode size={14} className="mr-2" /> Global Audit Ledger
               </button>
             )}
            </div>
         </div>
         <div className="max-h-[600px] overflow-y-auto scrollbar-thin scrollbar-thumb-[#333] scrollbar-track-transparent">
          <table className="w-full text-left text-sm whitespace-nowrap border-collapse">
            <thead className="sticky top-0 z-10 bg-[#111] border-b border-[#222] text-[11px] font-bold uppercase tracking-widest text-gray-500 shadow-sm">
              <tr>
                <th className="px-5 py-3">Agent</th>
                <th className="px-5 py-3">DID</th>
                <th className="px-5 py-3">Computational Latency</th>
                <th className="px-5 py-3">Integrity Signature</th>
              ...
              {agents.filter(a => 
                ...
                >
                  <td className="px-5 py-4 font-medium text-gray-200">
                    <div className="flex items-center">
                      <div className={cn("w-1.5 h-1.5 rounded-full mr-3", 
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center space-x-2">
                      <Zap size={10} className={cn(agent.last_latency > 500 ? "text-amber-500" : "text-emerald-500")} />
                      <span className={cn("font-mono text-[10px] font-bold", agent.last_latency > 500 ? "text-amber-500" : "text-emerald-400")}>
                        {agent.last_latency ? `${agent.last_latency.toFixed(1)}ms` : '---'}
                      </span>
                    </div>
                  </td>
                  <td className="px-5 py-4">
                    <div className="flex items-center bg-white/[0.03] border border-white/10 rounded-lg px-2 py-1 w-fit">
                      <ShieldCheck size={10} className="text-emerald-500 mr-2" />
                      <span className="font-mono text-[9px] text-gray-400 tracking-tighter">
                        {agent.last_hash ? agent.last_hash.substring(0, 12).toUpperCase() : 'NO_SIGNATURE_FOUND'}
                      </span>
                    </div>
                  </td>
                  <td className="px-5 py-4">
                    <div className="flex flex-col space-y-1.5 w-24">
                       <div className="flex justify-between text-[8px] font-bold text-gray-500 uppercase tracking-tighter">
                          ...
                          <span className="text-emerald-500">{agent.trust_index || 99.5}%</span>
                       </div>
                       <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                          <motion.div 
                            initial={{ width: 0 }}
                            animate={{ width: `${agent.trust_index || 99.5}%` }}
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center">
                      <div className={cn("w-1.5 h-1.5 rounded-full mr-2", 
                        ...
                        agent.status === 'Compromised' ? "bg-red-500 animate-pulse" : "bg-gray-500"
                      )} />
                    ...
                    <div className="flex justify-end items-center space-x-3">
                      {agent.status === 'Compromised' && (
                        <button 
                          ...
                          className="px-3 py-1 bg-indigo-600 text-white rounded-xl hover:bg-indigo-500 transition-all flex items-center text-[9px] font-black uppercase tracking-widest shadow-[0_0_10px_rgba(79,70,229,0.3)] border border-indigo-400/50"
                        >
                      ...
                      <div className="flex items-center space-x-1 border-l border-white/10 pl-3">
                        ...
                        {agent.alias !== 'ROOT_IDENTITY_ANCHOR' && (
                          <>
  ...
  const renderAddAgent = () => (
    <motion.div key="add-agent" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-8 w-full">
        <div className="flex justify-between items-end mb-4">
          <div className="text-right ml-auto">
             <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Genesis Subsidy</div>
             <div className={cn("text-xs font-mono font-bold px-3 py-1 border rounded-full", freeSlots > 0 ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : "bg-amber-500/10 text-amber-500 border-amber-500/20")}>
                {freeSlots > 0 ? `${freeSlots} SLOTS AVAILABLE` : "LIMIT REACHED - USING CREDIT"}
             </div>
          </div>
        </div>

      {provisionSuccess ? (
        <section className="bg-white/5 backdrop-blur-2xl border border-white/10 p-10 rounded-3xl text-center space-y-6 shadow-sm">
          ...
          <div className="space-y-2">
            <h3 className="text-xl font-bold text-white uppercase tracking-tighter">Anchor Deployed</h3>
            <p className="text-xs text-gray-400 font-mono uppercase tracking-widest">Digital ID Successfully Notarized</p>
          </div>
          <div className="p-4 bg-white/[0.03] border border-white/10 rounded-2xl font-mono text-[13px] text-emerald-400 break-all select-all">
            {provisionDid}
          </div>
          <div className="flex gap-4">
            <button 
              ...
              className="flex-1 py-3 bg-emerald-600 text-white rounded-2xl text-[13px] font-black uppercase tracking-widest hover:bg-emerald-500 transition-all shadow-[0_0_20px_rgba(16,185,129,0.2)] flex items-center justify-center"
            >
            ...
            <button 
              ...
              className="flex-1 py-3 bg-white text-black rounded-2xl text-[13px] font-black uppercase tracking-widest hover:bg-gray-200 transition-colors shadow-sm"
            >
        ...
        <div className="space-y-6">
            <div className="bg-white/5 border border-white/10 p-8 rounded-3xl shadow-2xl relative transition-all">
              <div className="space-y-8 relative z-10">
                <div className="space-y-3 flex flex-col">
                  ...
                  <input 
                    ...
                    className="w-full bg-white/[0.02] border border-white/10 px-4 py-4 text-sm font-bold text-white focus:outline-none focus:border-emerald-500/50 focus:bg-white/[0.05] transition-all rounded-2xl placeholder:text-gray-700 font-mono"
                  />
                </div>
                ...
                <div className="space-y-3 flex flex-col">
                  ...
                  <textarea 
                    ...
                    className="w-full bg-white/[0.02] border border-white/10 px-4 py-4 text-sm font-bold text-white focus:outline-none focus:border-emerald-500/50 focus:bg-white/[0.05] transition-all rounded-2xl h-48 resize-none placeholder:text-gray-700 leading-relaxed font-mono"
                  />
                </div>
              </div>
              ...
              <div className="pt-10 mt-10 border-t border-white/10">
                <button 
                  ...
                  className="w-full py-4 bg-white text-black rounded-2xl text-[12px] font-black uppercase tracking-[0.2em] hover:bg-gray-200 active:scale-[0.99] transition-all shadow-xl disabled:opacity-50 disabled:active:scale-100 flex items-center justify-center relative z-10"
                >
                  {isProvisioning ? (
                    <>
                      <RefreshCw size={16} className="mr-3 animate-spin" />
                      Initializing Cryptographic Anchor...
                    </>
                  ) : (freeSlots <= 0 && !isPaymentMethodActive) ? 'ADD AGENT HERE' : 'Deploy Identity Anchor'}
                </button>
              </div>
            </div>
        </div>
      ...
      <div className="min-h-screen bg-[#000000] text-gray-200 flex font-sans selection:bg-neutral-800 relative z-0">
        ...
        <aside className="w-64 border-r border-[#1a1a1a] flex flex-col shrink-0 bg-[#000000] z-20">
          <div className="h-20 flex items-center px-6 border-b border-[#1a1a1a]">
            ...
            <div className="flex flex-col justify-center">
              <div className="flex items-center space-x-2">
                ...
                <span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[7px] font-black bg-white/10 text-white border border-white/20 uppercase tracking-widest shadow-[0_0_10px_rgba(255,255,255,0.05)]">
                  Institutional Node Member
                </span>
              </div>
              <span className="text-[9px] text-gray-500 tracking-[0.2em] uppercase mt-1 font-mono">Protocol Node</span>
            </div>
          </div>
          ...
          <nav className="flex-1 px-3 py-6 space-y-8 overflow-y-auto">
            ...
            {/* Watchtower Section */}
            <div className="space-y-1">
              <div className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.25em] px-3 mb-3 text-white/40">Control Tower</div>
              <button onClick={() => { setActiveZone('tower'); setActiveView('dashboard'); }} className={cn("w-full flex items-center px-3 py-2.5 text-[13px] font-semibold rounded-xl transition-colors", activeView === 'dashboard' ? 'bg-white/5 text-white' : 'text-gray-400 hover:bg-white/[0.02]')}>
                <LayoutDashboard size={16} className="mr-3 text-gray-500" /> Dashboard
              </button>
              <button onClick={() => { setActiveZone('tower'); setActiveView('treasury'); }} className={cn("w-full flex items-center px-3 py-2.5 text-[13px] font-semibold rounded-xl transition-colors", activeView === 'treasury' ? 'bg-white/5 text-white' : 'text-gray-400 hover:bg-white/[0.02]')}>
                <DollarSign size={16} className="mr-3 text-gray-500" /> Treasury
              </button>
              <Link href="/dashboard/billing" className={cn("w-full flex items-center px-3 py-2.5 text-[13px] font-semibold rounded-xl transition-colors", activeView === 'billing' ? 'bg-white/5 text-white border-l-2 border-emerald-500' : 'text-gray-400 hover:bg-white/[0.02]')}>
                <ReceiptText size={16} className="mr-3 text-emerald-500" /> SVTP Billing
              </Link>
              <button onClick={() => { setActiveZone('tower'); setActiveView('approvals'); }} className={cn("w-full flex justify-between items-center px-3 py-2.5 text-[13px] font-semibold rounded-xl transition-colors", activeView === 'approvals' ? 'bg-white/5 text-white' : 'text-gray-400 hover:bg-white/[0.02]')}>
                <div className="flex items-center"><User size={16} className="mr-3 text-gray-500" /> Approvals</div>
              ...
              <button onClick={() => { setActiveZone('tower'); setActiveView('registry'); }} className={cn("w-full flex items-center px-3 py-2.5 text-[13px] font-semibold rounded-xl transition-colors", activeView === 'registry' ? 'bg-white/5 text-white' : 'text-gray-400 hover:bg-white/[0.02]')}>
                <Cpu size={16} className="mr-3 text-gray-500" /> Fleet Registry
              </button>

              <button 
                onClick={() => { setActiveZone('tower'); setActiveView('add-agent'); }} 
                className={cn("w-full flex items-center px-3 py-2.5 text-[13px] font-semibold rounded-xl transition-colors", activeView === 'add-agent' ? 'bg-[#1a1a1a] text-white border-l-2 border-[#CBFF00]' : 'text-gray-400 hover:bg-[#111]')}
            ...
            <div className="space-y-1">
              ...
              <button
                onClick={() => { setActiveZone('governance'); setActiveView('identity-vault'); }}
                className={cn(
                  "w-full flex items-center px-3 py-2 text-sm font-medium rounded-none transition-colors relative",
                  activeView === 'identity-vault' && activeZone === 'governance' ? 'bg-[#1a1a1a] text-white' : 'text-gray-400 hover:bg-[#111]'
                )}
                style={activeView === 'identity-vault' && activeZone === 'governance' ? { borderLeft: `2px solid ${branding?.brand_color || '#10B981'}`, color: branding?.brand_color || '#10B981' } : {}}
              >
                <Key size={16} className="mr-3" /> Identity Vault
                {activeView === 'identity-vault' && activeZone === 'governance' && (
                  <motion.div layoutId="sidebar-pill" className="absolute inset-0 bg-white/5 -z-10" />
                )}
              </button>
              <button
              ...
              >
                <Settings size={16} className="mr-3" /> SVTP Settings
              </button>
            </div>
          </nav>
        ...
        <main className="flex-1 flex flex-col h-screen overflow-hidden bg-[#050505] pt-16">
          <div className="flex-1 overflow-y-auto p-10 space-y-8">
                    ...
                    activeView === 'treasury' ? 'Financial Treasury' :
                      activeView === 'billing' ? 'Billing' :
                        ...
                        activeView === 'registry' ? 'Fleet Registry' :
                          activeView === 'add-agent' ? 'Add New Agent' :
                             activeView === 'installation' ? 'SDK Installation' :
                            activeView === 'integrations' ? 'System Integrations' :
                              activeView === 'apikeys' ? 'Node API Keys' :
                        activeView === 'settings' ? 'SVTP Settings' : 
                        activeView === 'identity-vault' ? 'Identity Vault' : 'Registry Watchtower'}
                </h2>
              </div>

              {activeView === 'dashboard' && (
                <div className="w-full lg:w-[380px] bg-[#050505] border border-white/10 rounded-none p-4 shadow-2xl relative overflow-hidden group hover:border-emerald-500/30 transition-all">
                  <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                    <Fingerprint size={80} />
                  </div>
            ...
            <AnimatePresence mode="wait">
              ...
              {activeZone === 'governance' && activeView === 'identity-vault' && renderIdentityVault()}
                  ...
                  <div className="pt-4 border-t border-[#1a1a1a]">
                    ...
                    <div className="space-y-3">
                      {(selectedAgent.reasoning_trace || [
                        ...
                        <div key={`trace-${i}`} className="flex items-start space-x-3">
          ...
          {showGlobalRevokeConfirm && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
              ...
              <motion.div 
                ...
                className="relative w-full max-w-xl bg-zinc-950/90 border border-red-500/20 rounded-[40px] p-12 shadow-[0_0_80px_rgba(239,68,68,0.15)] text-center space-y-10 backdrop-blur-xl"
              >
                <div className="mx-auto w-24 h-24 bg-red-500/10 rounded-full flex items-center justify-center border border-red-500/20 relative">
                  <div className="absolute inset-0 bg-red-500/5 rounded-full animate-ping" />
                  <OctagonAlert size={44} className="text-red-500 relative z-10" />
                </div>
                
                <div className="space-y-4">
                  <h2 className="text-[28px] font-bold text-white tracking-tight leading-tight">Initiate Global Revocation?</h2>
                  <p className="text-[15px] text-gray-400 leading-relaxed max-w-md mx-auto">
                    This will cryptographically terminate <span className="text-white font-bold">all active identities</span> across the Sovereign Topology. This action is <span className="text-red-500 font-bold uppercase tracking-widest text-xs">irreversible</span> and will be logged in the NIST audit trail.
                  </p>
                </div>
                ...
                <div className="flex gap-4 pt-4">
                  <button 
                    ...
                    className="flex-1 py-4 bg-white/5 border border-white/10 text-gray-300 text-sm font-semibold rounded-2xl hover:bg-white/10 hover:text-white transition-all"
                  >
                    Abort Protocol
                  </button>
                  <button 
                    ...
                    className={cn(
                      "flex-1 py-4 text-white text-sm font-bold rounded-2xl transition-all shadow-[0_10px_30px_rgba(239,68,68,0.3)] border border-red-500/50 flex items-center justify-center",
                      isGlobalRevoking ? "bg-red-950/50 cursor-not-allowed" : "bg-red-600 hover:bg-red-500"
                    )}
                  >
                    {isGlobalRevoking ? (
                      <>
                        <ShieldAlert size={18} className="mr-2 animate-pulse" />
                        TERMINATING FLEET...
                      </>
